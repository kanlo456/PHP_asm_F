...
if (isset($_POST["add_to_cart"])) {   //create 3D array to store shooping cart item
    if (isset($_SESSION["shopping_cart"])) {
        ...
        } else { // if item in the shopping cart, show message
...
<div class="container py-4 py-xl-5">
    <div class="row mb-5">
        <div class="col">
            ...
            <input class="float-end" type="search"></div>
    </div>
    <?php
    include "conn.php";
    $sql = "SELECT * FROM item"; //create product view
    $rs = mysqli_query($conn, $sql) or die(mysqli_error($conn));
    echo '<div class="row gy-4 row-cols-1 row-cols-md-2 row-cols-xl-3">';
    while ($rc = mysqli_fetch_assoc($rs)) {
        if ($rc['stockQuantity'] > 0) {
            echo <<<EOF
            <div class="col">
               <form method="post" action="productView.php?action=add&id={$rc['itemID']}">
                <div><img class="rounded img-fluid d-block w-100 fit-cover" style="height: 200px;" src="https://cdn.bootstrapstudio.io/placeholders/1400x800.png">
                    <div class="py-4">
                        <h4>{$rc['itemName']}</h4>
                        <p id="price1">$ {$rc['price']}</p>
                        <span>Stock Quantity:</span>
                        <span><input class="col-xl-2" type="number" readonly value="{$rc['stockQuantity']}"></span></br>
                        <span>Quantity: </span><input class="col-xl-2" type="number" value="1" min="1" max="{$rc['stockQuantity']}" name="quantity"></br>
                        Description:
                        <input type="hidden" name="itemName" value="{$rc['itemName']}">
                        <input type="hidden" name="price" value="{$rc['price']}">
                        <p>{$rc['itemDescription']}</p><input value="Add to cart" name="add_to_cart" type="submit"  class="btn btn-primary" >
                    </div>
            </form>
        </div>
    </div>
